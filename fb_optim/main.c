...
int fout = creat("out.file", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
printf("fout = %d\n", fout);
...
vc_dispmanx_rect_set(&rect1, 0, 0, 320, 240);
	...
	printf("------\n"); 
       vc_dispmanx_resource_read_data(screen_resource, &rect1, fbp, 320 * 16 / 8);//vinfo.xres * vinfo.bits_per_pixel / 8);
	printf("do\n");        
usleep(25 * 1000);
